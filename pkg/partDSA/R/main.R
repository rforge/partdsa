worker <- function(cv.ind, minbuck, cut.off.growth, MPD, missing,
                   loss.function, wt.method, brier.vec) {
 
   
  x <- data.frame(get('G.x')[get('G.grp.delt') != cv.ind,])
  if(inherits(get('G.y'),"Surv")) {  y <- get('G.y')[get('G.grp.delt') != cv.ind,]
  }else{ y <- get('G.y')[get('G.grp.delt') != cv.ind] }
  wt <- get('G.wt')[get('G.grp.delt') != cv.ind]

  x.test <- data.frame(get('G.x')[get('G.grp.delt') == cv.ind,])
  if(inherits(get('G.y'),"Surv")){  y.test <- get('G.y')[get('G.grp.delt') == cv.ind,]  }else{
    y.test <- get('G.y')[get('G.grp.delt') == cv.ind] }
  wt.test <- get('G.wt')[get('G.grp.delt') == cv.ind]


  # create weights for IPCW using assign.surv.wts function and the chosen wt.method
  if(loss.function=="IPCW"){
    wt=assign.surv.wts(x,y=Surv(y[,1],y[,2]),opts=list(loss.fx=loss.function, wt.method=wt.method))
    wt.test=assign.surv.wts(x=x.test,y=Surv(y.test[,1],y.test[,2]),opts=list(loss.fx=loss.function, wt.method=wt.method))
    
  }
 
 
 # for Brier, create weights using assign.surv.wts but now we need different weights depending on the brier.vec cutpoint. therefore
 # we create a matrix of weights where column i corresponds to the weights for the ith cutpoint 
  if(loss.function=="Brier"){

    wt=array(NA,c(nrow(x),length(brier.vec)))
    for (r in 1:length(brier.vec)){
      wt[,r]=assign.surv.wts(y=Surv(y[,1],y[,2]),opts=list(loss.fx=loss.function,wt.method=wt.method),T.star=brier.vec[r])
    }
    wt.test=array(NA,c(nrow(x.test),length(brier.vec)))
    for (r in 1:length(brier.vec)){
      wt.test[,r]=assign.surv.wts(y=Surv(y.test[,1],y.test[,2]),opts=list(loss.fx=loss.function,wt.method=wt.method),T.star=brier.vec[r])
    }
  }
  ...
  ty <- rss.dsa(x=x, y=y, wt=wt, minbuck=minbuck,
                ...
                loss.function=loss.function, wt.method=wt.method, brier.vec=brier.vec)
  ...
  if (ty$options$outcome.class == 'numeric' ) {
     ...
     # XXX not sure how to compute cv.risk when pred.test.DSA is a list of factors
     fun <- function(f) mean(f != y.test)
     cv.risk <- unlist(lapply(pred.test.DSA, fun))
  } else if (ty$options$outcome.class == 'survival' & ty$options$loss.fx == "IPCW") {
     tmp <- wt.test * ((y.test[,1] - pred.test.DSA) ^ 2)
     cv.risk <- apply(tmp, 2, sum) / sum(wt.test)
  } else if(ty$options$loss.fx=="Brier"){
       # for brier, calculating the risk is more complicated because we need to re-determine the coefficients and the wt depending on the cutpoint 
       # and then use that to calculate the risk. this function (in new functions) adds up the risk over all the cutpoint values
       cv.risk <- calculate.risk (model=ty, x, y, wt, x.test, y.test, wt.test, opts=list(loss.fx="Brier",brier.vec=brier.vec))
  }
...
partDSA <- function(x, y, wt=rep(1, nrow(x)), x.test=x, y.test=y, wt.test,
  ...
  #make the default value of brier.vec a single cutpoint equal to the median of y
  if((is.null(brier.vec)) & loss.function=="Brier"){brier.vec=median(y[,1])}
  ...
  if (length(which(missingness != dim(x)[1])) > 0 && missing == "no")
   stop(paste("Missing values found in dataset and missing is set to 'no'.",
              "Set missing='default' to proceed."))
  ...
  ## For survival, to create CV groups, use the same code as for a factor outcome but instead of using 
  ## y as the groups, use the censoring variable as the groups so that we have the same % of censoring in each fold.
  if (is.factor(y)){
  ...
  if (missing(sleigh)) {
    ...
    test.risk.DSA <- lapply(1:vfold, worker, minbuck, cut.off.growth,
                            MPD, missing, loss.function,wt.method, brier.vec)
  } else {
    ...
    test.risk.DSA <- eachElem(sleigh, worker, 1:vfold,
                              list(minbuck, cut.off.growth, MPD,
                                   missing, loss.function, wt.method, brier.vec))
  ...
  if(loss.function=="IPCW"){
    wt=assign.surv.wts(x=data.frame(x),y=Surv(y[,1],y[,2]),opts=list(loss.fx=loss.function, wt.method=wt.method))
    wt.test=assign.surv.wts(x=data.frame(x.test),y=Surv(y.test[,1],y.test[,2]),opts=list(loss.fx=loss.function, wt.method=wt.method))
  }
  ...
  }  else if (test2.ty$options$outcome.class == 'survival' & test2.ty$options$loss.fx=="IPCW") {
     tmp <- wt.test * (as.vector(y.test[,1]) - pred.test.set.DSA) ^ 2
     test.set.risk.DSA <- apply(tmp, 2, sum) / sum(wt.test)
  }  else if  (test2.ty$options$loss.fx=="Brier"){
      # separate function to calculate risk for brier because it needs to be summed over all the cutpoints in brier.vec
      test.set.risk.DSA <- calculate.risk (model=test2.ty,x,y,wt, x.test,y.test,wt.test, opts=list(loss.fx="Brier",brier.vec=brier.vec))
...
print.partDSA <- function(x, ...) {
  ...
  for (i in seq(along=x$mean.cv.risk)) {
